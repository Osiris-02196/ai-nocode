...
public interface TokenStream {
    ...
    /**
     * 用于接收工具执行请求的增量片段（旧版兼容回调）。
     * <p>
     * 新代码建议优先使用 {@link #onPartialToolCall(Consumer)}。
     * 当调用方仍以 {@link ToolExecutionRequest} 结构消费增量事件时，可使用此回调。
     *
     * @param toolExecutionRequestHandler 回调参数为工具调用索引与当前增量请求片段
     * @return 用于继续配置或启动流式处理的 token stream 实例
     */
    default TokenStream onPartialToolExecutionRequest(
