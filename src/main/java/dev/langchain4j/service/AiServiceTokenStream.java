...
public class AiServiceTokenStream implements TokenStream {
    ...
    public TokenStream onPartialToolExecutionRequest(
        ...
        // 兼容旧回调：支持仍以 ToolExecutionRequest 形态接收增量工具调用事件的调用方。
    ...
    public void start() {
        ...
        var handler = new AiServiceStreamingResponseHandler(
                ...
                partialToolCallHandler != null ? partialToolCallHandler : partialToolCall -> {
                    // 将新版 partialToolCall 事件桥接为旧版 onPartialToolExecutionRequest 回调。
                    if (partialToolExecutionRequestHandler != null) {
                        ToolExecutionRequest toolExecutionRequest = ToolExecutionRequest.builder()
                                ...
                                .build();
                        partialToolExecutionRequestHandler.accept(partialToolCall.index(), toolExecutionRequest);
                    }
                },
    ...
    private void validateConfiguration() {
        ...
        // 仅允许注册一种工具增量回调，避免同一 token 片段被重复下发。
        if (onPartialToolCallInvoked + onPartialToolCallWithContextInvoked + onPartialToolExecutionRequestInvoked > 1) {
            throw new IllegalConfigurationException(
                    "One of [onPartialToolCall, onPartialToolCallWithContext, onPartialToolExecutionRequest] "
                            + "can be invoked on TokenStream at most 1 time");
        }
